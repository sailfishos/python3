...
for file in %{buildroot}%{_libdir}/python%{python_version}/pydoc_data/topics.py $(grep --include='*.py' -lr %{buildroot}%{_libdir}/python%{python_version}/encodings -e 'Python Character Mapping Codec .* from .* with gencodec.py'); do
    directory=$(dirname ${file})
    module=$(basename ${file%%.py})
    mv ${directory}/{__pycache__/${module}.cpython-%{python_version_abitag}.pyc,${module}.pyc}
    rm ${directory}/{__pycache__/${module}.cpython-%{python_version_abitag}.opt-?.pyc,${module}.py}
done
